...
User_Book.findAll = function (id, result) {
  dbConn.query("select book.book_name,book.book_number_of_pages,book.book_date_of_issue,book.book_place_of_publication from book inner join user_on_loan on user_on_loan.ISBN_id=book.ISBN_id where user_on_loan.user_id= ?",[id], function (err, res) {
    if (err) {
      console.log('error: ', err)
      result(null, err)
    } else {
